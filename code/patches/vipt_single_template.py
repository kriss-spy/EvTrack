...
class ViPTTrack(BaseTracker):
    def __init__(self, params):
        ...
        # ---------- 新增：模板更新参数 ----------
        self.initial_z_tensor = None #保存初始模板
        self.z_tensor=None #输入模型的模板
        self.box_mask_z=None #输入模型的模板的mask
    ...
    def initialize(self, image, info: dict):
        ...
        # ---------- 保存初始模板 ----------
        self.initial_z_tensor = self.z_tensor.clone()
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.no_grad():
            out_dict = self.network.forward(
                template=self.z_tensor, search=search, ce_template_mask=self.box_mask_z)
        ...
        if (
            ...
            #第二种不断迭代更新
            self.z_tensor = self.z_tensor * (1 - max_score) + target_tensor * max_score

            if self.cfg.MODEL.BACKBONE.CE_LOC:
                new_bbox = self.transform_bbox_to_crop(self.state, target_resize_factor,
                                                       target_tensor.device).squeeze(1)
                self.box_mask_z = generate_mask_cond(self.cfg, 1, target_tensor.device, new_bbox)
